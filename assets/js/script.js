...
.map((item, i) =>    <article class="card" style="position:relative; overflow:hidden;">   <div class="ev-video-bg ${faqClasses[i % 4]}" style="opacity:0.05;"></div>   <h3 style="color:var(--text-main); margin-bottom:20px; position:relative; z-index:2; display:flex; gap:10px;">   <span style="color:var(--primary); font-family:'Playfair Display',serif; font-style:italic; font-weight:900;">Q.</span>   ${item.q}   </h3>   <div style="color:var(--text-dim); position:relative; z-index:2; padding-left:30px; border-left:1px solid var(--border);">   ${item.a}   </div>   </article>)
.join("");
}
