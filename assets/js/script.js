...
function toggleTheme() {
    ...
    if(btn) btn.innerText = document.body.classList.contains('light-mode') ? 'DARK MODE' : 'LIGHT MODE';
}
...
function init() {
    ...
    if(navLogo) navLogo.innerHTML = `${VAULT_DATA.owner.firstName} <span>${VAULT_DATA.owner.lastName}</span>`;
    if(heroBadge) heroBadge.innerText = VAULT_DATA.owner.badge;
    if(curatedBy) curatedBy.innerHTML = `Curated Selection by <strong>${VAULT_DATA.owner.fullName}</strong>`;
    ...
    setTimeout(() => typeWriter(VAULT_DATA.content.heroTitle, 0, () => renderProducts(VAULT_DATA.products)), 1000);
}
